...
parser.add_argument('--error_log_path', help='Error log txt file', default=None, type=str)
...
def train(device, model, train_data_loader, test_data_loader, optimizer,
          checkpoint_dir=None, checkpoint_interval=None, nepochs=None, error_log_path=None):
    ...
    while global_epoch < nepochs:
        ...
        for step, (x, mel, y) in prog_bar:
            ...
            if global_step % hparams.syncnet_eval_interval == 0:
                with torch.no_grad():
                    averaged_loss = eval_model(test_data_loader, global_step, \
                        device, model, checkpoint_dir)
                with open('eval_error.txt', 'a') as file:
                    file.write('{}, Loss: {}\n'.format(global_step, averaged_loss))
            with open(error_log_path, 'a') as file:
                file.write('{}, Running loss: {}, Loss: {}\n'.format(global_step, running_loss / (step + 1), loss.item()))
...
if __name__ == "__main__":
    ...
    train_dataset = Dataset('train', is_train=True)
    test_dataset = Dataset('train', is_train=False)
    ...
    train(device, model, train_data_loader, test_data_loader, optimizer,
          ...
          nepochs=hparams.nepochs, error_log_path=error_log_path)
